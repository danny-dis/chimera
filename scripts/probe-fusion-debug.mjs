...
const evs = [];
es.subscribe('*', (ev) => { if ((ev && ev.type || '').includes('error')) evs.push(ev.type + ':' + (ev.error || ev.message || '').slice(0, 80)); });
...
const t = Date.now();
const r = await o.execute({ task: 'There is a file bug.js with a deliberate bug (it adds instead of subtracts). Fix it so subtract(a,b) returns a-b. Write the corrected file.', mode: 'debug', providers, preset: 'fusion', costCap: 10 });
console.log(`code/fusion -> ${r.status} (${Date.now() - t}ms) ERR:${evs.join(' | ')} | out=${JSON.stringify(r.output).slice(0, 120)}`);
