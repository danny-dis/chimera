...
const costTracker = new (require('@chimera/providers').ProviderCostTracker)(reg);
const be = new BudgetEnforcer({ perTask: 10, perSession: 100, perDay: 500, alertThresholds: [0.5, 0.8] }, costTracker);
const rl = new RateLimiter({ rpm: 60, tpm: 1000000 });
async function run(mode, preset) {
  const es = new EventStream();
  const evs = [];
  es.subscribe('*', (ev) => { if ((ev && ev.type || '').includes('error')) evs.push((ev.type) + ':' + (ev.error || ev.message || '').slice(0, 140)); });
  ...
  const t = Date.now();
  const r = await o.execute({ task: 'Reply with exactly the single word: PONG', mode, providers, preset, costCap: 10 });
  console.log(`${mode}/${preset} -> ${r.status} (${Date.now() - t}ms) ERR:${evs.join(' | ')} | r.error=${(r.error||'')} | out=${(r.output||'').slice(0,120)}`);
}
await run('code', 'fusion');
await run('code', 'swarm');
