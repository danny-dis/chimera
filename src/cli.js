...
import { runAskMode, runCheckMode, runCodeMode, runPatchMode, runPlanMode, runReviewMode } from './modes.js';
...
Modes:
  ...
  patch <diff-file>   Validate a unified diff; add --apply to apply it
...
Options:
  ...
  --apply             Apply a patch after validation in patch mode
...
Examples:
  ...
  chimera check
  chimera patch .chimera/sessions/<id>/proposal.diff
  chimera review
`;
...
export async function runCli(args, io) {
  ...
  if (parsed.mode === 'ask') {
    ...
    result = await runPatchMode({ repo, prompt: parsed.prompt, session, selection, cwd: io.cwd, permissionProfile: parsed.permission, apply: parsed.apply });
