...
// ============================================================================
// CONTRACT LAYER — ported from research/archon/packages/providers/src/types.ts
//
// No SDK imports, no runtime deps. The types below mirror Archon's
// `IAgentProvider` contract — the streaming chunk union, capability flags, and
// request options that provider implementations and consumers agree on. This
// file must NEVER import SDK packages (`@anthropic-ai/sdk`, `@openai/codex-sdk`,
// etc.) or other @chimera/* packages.
//
// The shape here is additive to the existing ModelProvider abstraction above;
// both coexist because they serve different layers (provider-config surface
// vs. streaming message-chunk surface).
//
// Ported from research/archon/packages/providers/src/types.ts @ 2026-06-15.
// Slimmed for chimera week-2:
//   - dropped the `workflow_dispatch` MessageChunk variant (no workflow engine
//     wired into chimera-providers yet) — see TODO below
//   - dropped the `agents` field on NodeConfig (chimera's DAG schema lives in
//     chimera-workflows, not in providers)
//   - renamed the Archon `TokenUsage` to `MessageTokenUsage` to avoid colliding
//     with the existing `TokenUsage` (different field shape: `input`/`output`
//     vs. `inputTokens`/`outputTokens`). Both are exported from the barrel.
// ============================================================================
