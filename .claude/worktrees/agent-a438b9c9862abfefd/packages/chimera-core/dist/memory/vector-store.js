...
exports.VectorStore = exports.LocalEmbeddingProvider = void 0;
/**
...
class VectorStore {
    ...
    async search(query) {
        ...
        if (query.tagsFilter && query.tagsFilter.length > 0) {
            candidates = candidates.filter((item) => query.tagsFilter.some((tag) => item.metadata.tags.includes(tag)));
        }
        if (query.since) {
     ...
     */
    serialize() {
        return JSON.stringify(this.items.map(({ embedding, ...rest }) => rest), null, 2);
    }
    /**
