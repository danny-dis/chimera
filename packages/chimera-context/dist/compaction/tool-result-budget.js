...
function applyToolResultBudget(messages) {
    ...
    const result = messages.map((msg) => {
        ...
        if (byteSize > thresholds_js_1.TOOL_RESULT_MAX_BYTES || lineCount > thresholds_js_1.TOOL_RESULT_MAX_LINES) {
            const lines = msg.content.split('\n');
            const keptLines = lines.slice(0, thresholds_js_1.TOOL_RESULT_MAX_LINES);
            const keptBytes = Buffer.byteLength(keptLines.join('\n'), 'utf-8');
            if (keptBytes > thresholds_js_1.TOOL_RESULT_MAX_BYTES) {
                let byteCount = 0;
                let cutLine = 0;
                for (let i = 0; i < keptLines.length; i++) {
                    byteCount += Buffer.byteLength(keptLines[i], 'utf-8') + 1;
                    if (byteCount > thresholds_js_1.TOOL_RESULT_MAX_BYTES)
                        break;
                    cutLine = i;
                }
