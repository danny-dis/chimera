...
class ContextEngine {
    ...
    beta;
    gamma;
    constructor(workspaceRoot, instructionsFile, config = {}) {
        this.workspaceRoot = workspaceRoot;
        this.instructionsFile = instructionsFile;
        this.embeddingProvider = config.embeddingProvider;
        this.vectorStore = config.vectorStore ?? new vector_store_js_1.VectorStore();
    ...
    async buildVectorIndex() {
        ...
        const docs = Array.from(this.repoIndex.files.keys()).map(f => {
            try {
                return (0, fs_1.readFileSync)(path_1.default.join(this.workspaceRoot, f), 'utf-8');
            }
            catch {
                return '';
            }
...
- No circular dependencies.`;
    ...
    async buildContextPack(params) {
        ...
        for (const file of this.repoIndex.files.keys()) {
            ...
            const relevance = this.alpha * kwNorm + this.beta * embNorm + this.gamma * centNorm;
            if (relevance > 0)
