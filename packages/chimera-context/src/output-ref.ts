...
// ---------------------------------------------------------------------------
//
// A minimal, structurally-compatible shape for the producer's output.
// Chimera's quality-gate pipeline already produces objects of this shape
// (see `chimera-core/src/types/events.ts` for the wider event stream; the
// `final_response` events carry the producer's text + optional structured
// payload). This file does not import the event types to keep the resolver
// a pure leaf that any package can depend on.
