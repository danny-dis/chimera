...
exports.ArtifactImprover = void 0;
// ---------------------------------------------------------------------------
// Skill effectiveness analysis
// ---------------------------------------------------------------------------
function detectSkillToolMisalignment(skill, patterns) {
    const issues = [];
    ...
    const skillMentionsTool = (tool) => skill.content.toLowerCase().includes(tool.toLowerCase());
...
function detectSkillCostMisalignment(skill, patterns) {
    const issues = [];
...
function detectSkillQualityIssues(_skill, patterns) {
    const issues = [];
...
// ---------------------------------------------------------------------------
function detectWorkflowDeviations(workflow, patterns) {
    const issues = [];
...
class ArtifactImprover {
     ...
     */
    analyzeSkillEffectiveness(skill, patterns) {
        const issues = [
     ...
     */
    analyzeWorkflowEffectiveness(workflow, patterns) {
        const issues = [
     ...
     */
    improveSkill(skill, signal, patterns) {
     ...
     */
    improveWorkflow(workflow, signal, patterns) {
        const issues = signal.issues;
...
exports.ArtifactImprover = ArtifactImprover;
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeMoreActionable(content) {
...
function rebuildStepsFromSequence(sequence, existingSteps) {
    const steps = [];
    let counter = 0;
    for (const tool of sequence) {
        const existing = existingSteps.find(s => s.kind === 'tool' && s.config.name === tool);
        if (existing) {
            steps.push({ ...existing });
        }
        else {
...
//# sourceMappingURL=artifact-improver.js.map