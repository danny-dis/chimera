...
// ---------------------------------------------------------------------------
function inferToolSpecialization(patterns) {
...
function inferContextSpecialization(patterns) {
    // Merge file types
...
function inferModelTier(patterns) {
...
// ---------------------------------------------------------------------------
function generateSkillName(topic, patterns) {
...
function generateSkillDescription(topic, patterns) {
    const avgCost = patterns.reduce((sum, p) => sum + p.cost.totalUsd, 0) / patterns.length;
    ...
    return (`${topic.charAt(0).toUpperCase() + topic.slice(1)} implementation and review patterns. ` +
...
function generateSkillContent(topic, _cluster, patterns) {
...
class SkillSynthesizer {
     ...
     */
    improveExisting(existing, patterns) {
...
// Helpers
// ---------------------------------------------------------------------------
function mergeToolFrequency(patterns) {
    const merged = {};
    for (const p of patterns) {
        for (const [tool, count] of Object.entries(p.tools.frequency)) {
