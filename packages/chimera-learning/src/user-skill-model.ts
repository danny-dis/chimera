...
const SIGNAL_DELTA: Record<SkillSignal, number> = {
  'advanced-flag': +0.12,
  'scripted-usage': +0.10,
...
export class UserSkillModel {
  ...
  private samples = 0;
  private override: SkillOverride | null = null;
  private readonly audit: SkillAuditEntry[] = [];
  ...
  observeCommandUsage(usage: {
    ...
    const advancedFlags = (usage.flags ?? []).filter((f) =>
      /^(--preset|--no-learn|--verbose|--repl|--real|--force|-y)$/.test(f),
    );
    if (advancedFlags.length > 0) {
      this.observeSignal('advanced-flag', `advanced flags: ${advancedFlags.join(', ')}`);
    }
    if (usage.usedPreset) this.observeSignal('config-override', 'used deliberation preset');
    if (usage.scripted) this.observeSignal('scripted-usage', 'scripted/non-interactive usage');
    if (usage.configOverridden) this.observeSignal('config-override', 'overrode config');
  }
  ...
  tier(): SkillTier {
    if (this.override) return this.override === 'more' ? 'beginner' : 'advanced';
    if (this.evidenceConfidence < 1) {
      // Not enough signal yet — never assume novice or expert.
      return 'intermediate';
    }
    const s = this.score;
    if (s < 0.35) return 'beginner';
    if (s > 0.7) return 'advanced';
    return 'intermediate';
  }

  /** How much hand-holding to attach to a message. */
  explainDepth(): ExplainDepth {
    if (this.override === 'more') return 'full';
    if (this.override === 'less') return 'minimal';
    switch (this.tier()) {
      case 'beginner':
        return 'full';
      case 'advanced':
  ...
  tierReason(): string {
    if (this.override) return `explicit override="${this.override}"`;
    if (this.evidenceConfidence < 1) {
  ...
  formatAudit(): string {
    return this.audit
      ...
      .map(
        (e) =>
          `[${new Date(e.ts).toISOString()}] ${e.signal} Δ${e.delta >= 0 ? '+' : ''}${e.delta.toFixed(2)} → ${e.scoreAfter.toFixed(2)} (${e.tier}) — ${e.reason}`,
      )
      .join('\n');
  }
  ...
  private applyDelta(signal: SkillSignal | 'message', delta: number, reason: string): void {
 ...
 */
export function skillTierFromCli(args: string[] = process.argv.slice(2)): SkillTier {
