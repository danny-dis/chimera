/**
 * SessionAnalyzer — extracts structured patterns from completed sessions.
 *
 * Reads SessionCheckpoint data (EventStream + messages + costs) and produces
 * SessionPattern objects that the synthesizers consume.
 *
 * Extraction pipeline:
 *   1. Domain detection — keywords from task + file paths + tool call args
 *   2. Tool sequence extraction — ordered, deduped tool call names
 *   3. Quality extraction — verdicts, status, revision cycles
 *   4. Cost extraction — per-provider spend, efficiency classification
 *   5. Agent behavior — count, roles, handoffs
 */
...
const FILE_TYPE_DOMAINS: Record<string, string[]> = {
  '.sql': ['database'],
  '.prisma': ['database'],
  '.graphql': ['api'],
...
function extractToolPattern(events: ChimeraEvent[]): ToolPattern {
...
function extractQuality(events: ChimeraEvent[]): QualityOutcome {
  ...
  const lastVerdict = verifiedEvents.length > 0
    ? verifiedEvents[verifiedEvents.length - 1].verdict.toUpperCase() as QualityOutcome['verdict']
    : undefined;

  const revisionCycles = verifiedEvents.filter(e => e.verdict === 'needs_revision').length;

  const failures: QualityOutcome['failures'] = [];
...
function extractCost(_events: ChimeraEvent[], costSpend: Record<string, number>): CostProfile {
  const totalUsd = Object.values(costSpend).reduce((sum, c) => sum + c, 0);

  // Classify efficiency (rough heuristic)
  let efficiency: CostProfile['efficiency'] = 'moderate';
  if (totalUsd < 0.10) efficiency = 'cheap';
  else if (totalUsd > 1.00) efficiency = 'expensive';
...
function extractAgentBehavior(events: ChimeraEvent[]): AgentBehavior {
...
export class SessionAnalyzer {
  ...
  analyzeBatch(checkpoints: SessionCheckpoint[]): ClusteredPatterns {
    ...
    // Group by domain
    const byDomain = new Map<string, SessionPattern[]>();
    ...
    const bySequence = new Map<string, SessionPattern[]>();
    for (const p of patterns) {
    ...
    // Group by outcome
    const byOutcome = new Map<string, SessionPattern[]>();
    for (const p of patterns) {
