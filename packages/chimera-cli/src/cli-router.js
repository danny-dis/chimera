...
class CliRouter {
    ...
    async buildRunContext() {
        ...
        const orchestrator = new core_1.SessionOrchestrator(eventStream, { registry: toolRegistry, executor: toolExecutor }, process.cwd(), undefined, {
            ...
            budgetEnforcer,
            rateLimiter,
            memoryPersistence: new core_1.MemoryPersistence({ workspaceRoot: process.cwd() }),
        });
        return {
            orchestrator,
            writer,
    ...
    async startTui(initialTask, initialMode = 'code') {
        ...
        /** Build cost data, deriving the live cumulative spend from the orchestrator's cost tracker. */
        const buildLiveCostData = () => {
            ...
            // Derive a per-role breakdown from actual agent token usage (the real
            // signal). CostTracker keys spend by provider, not role, so we cannot
            // attribute cost to a role directly — only token counts are per-agent.
            const roles = Array.from(new Set(currentAgents.map((a) => a.role)));
            const breakdown = roles.map((role) => {
                const agentsForRole = currentAgents.filter((a) => a.role === role);
                const sample = agentsForRole[0];
                return {
                    provider: sample?.provider ?? 'default',
                    model: sample?.model ?? 'default',
                    inputTokens: agentsForRole.reduce((s, a) => s + a.tokenUsage.input, 0),
                    outputTokens: agentsForRole.reduce((s, a) => s + a.tokenUsage.output, 0),
                    cost: 0,
                };
            });
            return {
