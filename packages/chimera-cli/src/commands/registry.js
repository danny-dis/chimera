...
const custom_loader_js_1 = require("./custom-loader.js");
...
const HELP_TEXT = [
    ...
    '',
    '  Settings:',
    '    /model /status /config /doctor /export /setup',
 ...
 */
async function printHelp(_ctx) {
    const customCmds = await (0, custom_loader_js_1.loadCustomCommands)().catch(() => new Map());
 ...
 */
async function runSlashCommand(cmd, args, ctx) {
    switch (cmd) {
        case 'mode': {
...
async function handleLoop(ctx, turns, task) {
...
async function handleGoal(ctx, goal) {
...
function handleCost(ctx) {
    ...
    const aggregate = tracker.getTotal?.() ?? null;
...
async function handleInit(args) {
    ...
    try {
        const res = await (0, init_js_1.initAgentsMd)(process.cwd(), { force });
        console.log(`Wrote AGENTS.md (${res.bytesWritten} bytes). Review and edit before committing.`);
    }
    catch (err) {
...
async function handleRefreshModels(args) {
    const force = args.includes('--force');
    const fetcher = new providers_1.ModelMetadataFetcher();
...
async function handleModel(ctx, args) {
    ...
    const match = providers.find((p) => p.getModel().name.toLowerCase().includes(wanted) ||
        p.getModel().id.toLowerCase().includes(wanted));
    if (match) {
        const m = match.getModel();
        console.log(`  Switched writer to ${m.provider}/${m.name} (note: orchestrator picks the cheapest non-mock provider; this just confirms availability)`);
    }
    else {
...
async function handleResume(ctx, args) {
    if (args.length === 0) {
    ...
    ctx.setMode(cp.mode);
    const orchestrator = ctx.currentOrchestrator ?? (await ctx.initOrchestrator());
    // core owns restoreState; the checkpoint shape is already consolidated.
    await orchestrator.restoreState(cp);
    ctx.setCurrentOrchestrator(orchestrator);
    const turnCount = cp.metadata?.turnCount ?? 0;
    const toolCalls = cp.toolCallHistory?.length ?? 0;
    console.log(`  Restored ${toolCalls} tool calls, ${turnCount} turns.`);
    console.log(`  Run /rewind or send a message to continue from this checkpoint.`);
    return 'continue';
}
...
async function handleConfig(_ctx) {
    try {
        const fs = await import('fs');
        const path = await import('path');
...
async function handleRewind(ctx, args) {
    const sessions = await ctx.getSessionStore().list();
    if (sessions.length === 0) {
...
function handleOutputStyle(args) {
...
function handlePermissions(ctx) {
...
function handleVim() {
    ...
    try {
        ...
        const current = config.vim ?? false;
        config.vim = !current;
        const yaml = require('yaml');
...
function handleHooks() {
    ...
    try {
        ...
        if (fs.existsSync(hooksDir)) {
            const files = fs.readdirSync(hooksDir).filter((f) => f.endsWith('.sh') || f.endsWith('.js') || f.endsWith('.ts'));
            if (files.length > 0) {
...
function handleMigrateInstaller() {
    const fs = require('fs');
    const path = require('path');
...
async function handleSetup() {
    ...
    console.log('  Restart the REPL to apply new provider settings.');
    return 'continue';
}
async function handleUnknown(ctx, cmd, args) {
    // Try custom commands from .chimera/commands/ and ~/.chimera/commands/
    // before falling back to the "Unknown command" message.
    const custom = await (0, custom_loader_js_1.loadCustomCommands)().catch(() => new Map());
    if (custom.has(cmd)) {
        const customOrch = ctx.currentOrchestrator
            ...
            : null;
        await (0, custom_loader_js_1.runCustomCommand)(cmd, args, customOrch);
        return 'continue';
    }
