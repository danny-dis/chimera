...
exports.classifyIsolationError = exports.WorktreeProvider = void 0;
const promises_1 = require("node:fs/promises");
const node_path_1 = require("node:path");
const paths_1 = require("@chimera/paths");
const worktree_copy_js_1 = require("../worktree-copy.js");
...
const worktree_helpers_js_1 = require("./worktree-helpers.js");
const branded_js_1 = require("../types/branded.js");
let cachedLog;
function getLog() {
    if (!cachedLog)
        cachedLog = (0, paths_1.createLogger)('isolation.worktree');
    return cachedLog;
}
// ---------------------------------------------------------------------------
...
async function git(args, cwd, timeoutMs = GIT_OPERATION_TIMEOUT_MS) {
    try {
...
class WorktreeProvider {
    ...
    constructor(loadConfig = defaultLoadConfig) {
    ...
    async create(request) {
        ...
        const branchName = (0, branded_js_1.toBranchName)(this.generateBranchName(request));
    ...
    async destroy(envId, options) {
        ...
        if (worktreeOptions?.canonicalRepoPath) {
            repoPath = worktreeOptions.canonicalRepoPath;
        }
        else if (pathExists) {
        ...
        if (result.worktreeRemoved) {
            ...
            if (stillRegistered) {
                ...
                result.warnings.push(`Worktree at ${worktreePath} was reported removed but is still registered in git`);
            }
        }
        // Delete associated branch if provided (best-effort cleanup).
        if (worktreeOptions?.branchName) {
            result.branchDeleted = await this.deleteBranchTracked(repoPath, worktreeOptions.branchName, result);
            if (worktreeOptions.deleteRemoteBranch) {
                result.remoteBranchDeleted = await this.deleteRemoteBranchTracked(repoPath, worktreeOptions.branchName, result);
            }
        }
        return result;
    }
    ...
    async get(envId) {
        const worktreePath = envId;
        if (!(await this.worktreeExists((0, branded_js_1.toWorktreePath)(worktreePath)))) {
            return null;
        }
        let repoPath;
        let worktrees;
        ...
        return {
            ...
            provider: 'worktree',
            workingPath: worktreePath,
            branchName: (0, branded_js_1.toBranchName)(wt.branch),
            status: 'active',
     ...
     */
    async adopt(path) {
        if (!(await this.worktreeExists((0, branded_js_1.toWorktreePath)(path)))) {
            return null;
        }
        let repoPath;
        let worktrees;
    ...
    async healthCheck(envId) {
        return this.worktreeExists((0, branded_js_1.toWorktreePath)(envId));
    }
    // =========================================================================
     ...
     */
    getWorktreePath(request, branchName, config) {
        const repoRoot = request.canonicalRepoPath;
        const override = (0, worktree_helpers_js_1.resolveRepoLocalOverride)(config?.path, repoRoot);
        if (override !== undefined) {
            return (0, node_path_1.join)(repoRoot, override, branchName);
        }
        // Default layout: under the repo's own `.chimera-worktrees/`.
        return (0, node_path_1.join)(repoRoot, '.chimera-worktrees', branchName);
    }
    // =========================================================================
    // Private helpers
    // =========================================================================
    async findExisting(request, branchName, worktreePath) {
        if (await this.worktreeExists((0, branded_js_1.toWorktreePath)(worktreePath))) {
            return this.buildAdoptedEnvironment(worktreePath, branchName, request);
        }
        return null;
    }
    buildAdoptedEnvironment(path, branchName, request) {
        return {
            id: path,
            provider: 'worktree',
            workingPath: path,
            branchName: (0, branded_js_1.toBranchName)(branchName),
            status: 'active',
            createdAt: new Date(),
     ...
     */
    async createWorktree(request, worktreePath, branchName, worktreeConfig) {
        const repoPath = request.canonicalRepoPath;
        const baseBranch = worktreeConfig?.baseBranch ?? 'HEAD';
        // Ensure the parent directory exists.
        const worktreeBase = (0, node_path_1.normalize)((0, node_path_1.join)(worktreePath, '..'));
        await this.ensureDir(worktreeBase);
        // Determine start-point: explicit fromBranch overrides base branch.
        const startPoint = request.workflowType === 'task' && request.fromBranch
            ? request.fromBranch
            : baseBranch;
        ...
        try {
            ...
            if (err.stderr?.includes('already exists')) {
                ...
                if (request.workflowType === 'task' && request.fromBranch) {
                    throw new Error(`Branch "${branchName}" already exists. Cannot create it from "${request.fromBranch}". ` +
        ...
        const { configLoadFailed } = await this.copyConfiguredFiles(repoPath, worktreePath, worktreeConfig);
        const warnings = [];
        if (configLoadFailed) {
            warnings.push('Config file could not be loaded — copyFiles configuration was not applied. Check your .archon/config.yaml for syntax errors.');
        }
        return { warnings };
    }
    /**
     ...
     */
    async applyGitIdentity(worktreePath, identity) {
        try {
     ...
     */
    async initSubmodules(worktreePath) {
        try {
            await (0, promises_1.access)((0, node_path_1.join)(worktreePath, '.gitmodules'));
        }
        catch (error) {
            const err = error;
            if (err.code === 'ENOENT') {
            ...
            throw new Error(`Submodule initialization failed: cannot read .gitmodules (${err.code ?? 'unknown error'})`);
        }
        try {
            await git(['-C', worktreePath, 'submodule', 'update', '--init', '--recursive'], worktreePath, 120000);
        }
        catch (error) {
            const err = error;
     ...
     */
    async copyConfiguredFiles(canonicalRepoPath, worktreePath, worktreeConfig) {
        ...
        const result = await (0, worktree_copy_js_1.copyWorktreeFiles)(canonicalRepoPath, worktreePath, copyFiles);
        if (result.copied.length < copyFiles.length) {
            ...
            getLog().warn({ copied: result.copied.length, requested: copyFiles.length }, (0, paths_1.logEvent)('worktree', 'copy', 'partial'));
        }
        return { configLoadFailed };
    }
    /**
    ...
    async directoryExists(path) {
        try {
            ...
            throw new Error(`Failed to check directory at ${path}: ${err.message} (code: ${err.code ?? 'unknown'})`);
        }
    }
     ...
     */
    async getCanonicalRepoPath(worktreePath) {
     ...
     */
    async isWorktreeRegistered(repoPath, worktreePath) {
        try {
            const { stdout } = await git(['-C', repoPath, 'worktree', 'list', '--porcelain'], repoPath, 15000);
            const normalizedTarget = (0, node_path_1.resolve)(worktreePath);
            return stdout.split('\n').some((line) => {
                if (!line.startsWith('worktree '))
                    return false;
                const listed = line.slice('worktree '.length).trim();
                return (0, node_path_1.resolve)(listed) === normalizedTarget;
            });
        }
        catch {
            return false;
    ...
    isWorktreeMissingError(error) {
        ...
        return (errorText.includes('No such file or directory') ||
     ...
     */
    async deleteBranchTracked(repoPath, branchName, result) {
        try {
            ...
            if (errorText.includes('not found') || errorText.includes('did not match any')) {
                ...
                result.warnings.push(`Cannot delete branch '${branchName}': branch is checked out elsewhere`);
                return false;
            }
            else {
     ...
     */
    async deleteRemoteBranchTracked(repoPath, branchName, result) {
        try {
            ...
            const err = error;
            const errorText = `${err.message} ${err.stderr ?? ''}`;
            if (errorText.includes('remote ref does not exist') ||
                errorText.includes("couldn't find remote ref")) {
                return true;
            }
            else {
                result.warnings.push(`Failed to delete remote branch '${branchName}': ${err.message}`);
                return false;
            }
        }
    }
     ...
     * (like `.archon/`) behind.
     */
    async cleanOrphanDirectoryIfExists(worktreePath) {
        const dirExists = await this.directoryExists(worktreePath);
        if (!dirExists)
            return;
        const isValidWorktree = await this.worktreeExists((0, branded_js_1.toWorktreePath)(worktreePath));
        if (isValidWorktree)
            return;
        try {
            await (0, promises_1.rm)(worktreePath, { recursive: true, force: true });
    ...
    slugify = (input) => (0, worktree_helpers_js_1.slugify)(input);
    shortHash = (input) => (0, worktree_helpers_js_1.shortHash)(input);
}
 ...
 */
async function defaultLoadConfig(repoPath) {
 ...
 *     path: .worktrees
 */
function parseWorktreeConfigBlock(raw) {
    ...
    if (copyFiles !== undefined)
        result.copyFiles = copyFiles;
    if (initSubmodules !== undefined)
        result.initSubmodules = initSubmodules;
    if (path !== undefined)
        result.path = path;
    return result;
}
