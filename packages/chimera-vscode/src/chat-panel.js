...
class ChatPanel {
    ...
    static createOrShow(context, daemon) {
        ...
        const panel = vscode.window.createWebviewPanel('chimeraChat', 'Chimera AI', column ?? vscode.ViewColumn.Beside, {
            enableScripts: true,
...
<body>
  ...
  <script>
    ...
    renderMessage(msg) {
        ...
        const content = escaped.replace(/\/(ask|code|plan|review|debug|oal)/g, '<span class="mode-badge mode-$1">$1</span>');
        // Convert ```code blocks
