...
class ConfigPanel {
    ...
    context;
    daemon;
    constructor(panel, context, daemon) {
        this.panel = panel;
    ...
    static createOrShow(context, daemon) {
        if (ConfigPanel.current) {
            ConfigPanel.current.panel.reveal();
            return ConfigPanel.current;
        }
        const panel = vscode.window.createWebviewPanel('chimeraConfig', 'Chimera Setup', vscode.ViewColumn.Active, {
            enableScripts: true,
        ...
        try {
            ...
            for (const p of config.providers) {
                if (p.apiKey) {
                    ...
                    await vscode.env.clipboard.writeText(`Set environment variable ${envKey}=${p.apiKey}`);
                }
            }
            vscode.window.showInformationMessage('Chimera configuration saved! Remember to set API keys as environment variables.');
            this.panel.dispose();
        }
        catch (err) {
