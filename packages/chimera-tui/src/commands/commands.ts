...
// ── Session summary (avoids importing CheckpointStore) ──────────────────
...
export interface CommandContext {
  ...
  // ── Optional: CLI-provided capabilities ──────────────────────────────
  // These are populated by the CLI registry adapter but absent in the TUI.
  // Commands that need them degrade gracefully when missing.
  ...
  /** Token usage by role. */
  getTokenUsage?: () => Array<{ role: string; spend: number }>;
  ...
  loadSession?: (id: string) => Promise<{ task: string; mode: string } | null>;
  /** Read memory entries. */
  getMemoryEntries?: () => Array<{ content: string; metadata?: { topic?: string } }>;
  /** Memory entry count. */
  getMemorySize?: () => number;
  /** Session store count. */
  getSessionCount?: () => Promise<number>;
  /** Provider list for /model, /doctor. */
  getProviders?: () => Promise<Array<{ getModel(): { provider: string; name: string; id: string; contextWindow: number } }>>;
  /** Orchestrator active? */
  hasOrchestrator?: () => boolean;
  /** Event stream for /tasks, /agents. */
  getEventStream?: () => { getAll: () => Array<{ type: string; [key: string]: unknown }> } | null;
  /** Init AGENTS.md. */
  initAgentsMd?: (cwd: string, opts?: { force?: boolean }) => Promise<{ bytesWritten: number }>;
  /** Run health checks. */
...
export function runCommand(
  ...
  switch (cmd) {
    ...
    case 'mode': {
      ...
      if (args[0] && known.includes(args[0] as Mode)) {
    ...
    case 'preset': {
      const known: DeliberationMode[] = ['solo', 'duo', 'trio', 'hive', 'fusion'];
      if (args[0] && known.includes(args[0] as DeliberationMode)) {
...
export function autocompleteCommand(partial: string): string[] {
  ...
  if (fragment === '') return ALL_COMMANDS.map((c) => `/${c}`);
