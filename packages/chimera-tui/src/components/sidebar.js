...
const Section = ({ label, color = zen.fg, children, }) => (<Box flexDirection="column" marginTop={1}>
...
export const Sidebar = ({ sessionId, mode, preset, agents, costData, tokenUsage, instructions, contentWidth, onModeChange, onPresetChange, }) => {
    ...
    return (<Box flexDirection="column" paddingX={1}>
      ...
      <Section label="Mode" color={zen.accent}>
        <ModeSelector mode={mode} compact focused onSelect={onModeChange}/>
      </Section>

      {/* Preset — interactive */}
      <Section label="Preset" color={zen.agent}>
        <PresetSelector preset={preset} compact focused onSelect={onPresetChange}/>
      </Section>
      ...
      <Section label="Capabilities">
        {['writer', 'reviewer', 'challenger', 'synthesizer'].map((r) => (<Box key={r}>
