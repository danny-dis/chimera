...
export const DiffViewer = ({ files }) => {
    ...
    return (<Box flexDirection="column" borderStyle="double" borderColor={zen.error} paddingX={1}>
      <Box marginBottom={1}>
      ...
      {files.map((file, i) => (<Box key={file.path} flexDirection="column">
          <FileHeader file={file} isExpanded={expandedFiles.has(i)} isSelected={i === selectedIndex}/>
          {expandedFiles.has(i) &&
                file.hunks.map((hunk, j) => (<Box key={j} flexDirection="column" marginLeft={2}>
                <Text dimColor>
                  @@ -{hunk.oldStart},{hunk.oldLines} +{hunk.newStart},{hunk.newLines} @@
                </Text>
                {hunk.lines.map((line, k) => (<DiffLineComponent key={k} lineNum={line.newLineNum} type={line.type} content={line.content}/>))}
              </Box>))}
        </Box>))}
