...
const AgentRow = ({ agent, contentWidth }) => {
    ...
    return (<Box flexDirection="column" marginBottom={1}>
      <Box>
        ...
        {agent.progress !== undefined && (<Text color={zen.accent}> [{Math.round(agent.progress * 100)}%]</Text>)}
      </Box>
      {showDetails && (<Box marginLeft={2}>
...
export const AgentDashboard = ({ agents, contentWidth }) => {
    ...
    return (<Box flexDirection="column" borderStyle="single" borderColor={zen.borderActive} paddingX={1}>
      <Box marginBottom={1}>
      ...
      <Box flexDirection="column" marginBottom={1}>
        ...
        {agents.map((agent) => (<AgentRow key={agent.id} agent={agent} contentWidth={contentWidth}/>))}
      </Box>

      <Box flexDirection="column" marginBottom={1}>
        <Text bold>Role capabilities</Text>
        {AGENT_CAPABILITIES.map((capability) => (<CapabilityRow key={capability.role} capability={capability} compact={compact}/>))}
      </Box>

      {!compact && (<Box flexDirection="column">
          <Text bold>Execution presets</Text>
          {PRESET_CAPABILITIES.map((preset) => (<Box key={preset.preset}>
