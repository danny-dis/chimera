...
export const PresetSelector = ({ preset, onSelect, focused = false, compact = false, contentWidth, }) => {
    ...
    if (compact) {
        ...
        {PRESETS.map((p) => {
                const sel = p === preset;
                return (<Box key={p} marginRight={1}>
              <Text color={sel ? zen.agent : zen.muted} bold={sel} underline={sel}>
    ...
    return (<Box flexDirection="column" borderStyle="single" borderColor={zen.agent} paddingX={1}>
