...
export function Viewport({ items, height, renderItem, getItemHeight, focused = false, autoScroll = true, }) {
    ...
    return (<Box flexDirection="column" height={height} overflow="hidden" width="100%">
      {/* Top padding: skip rows above the first visible item. */}
      {visibleStart > 0 && (<Box flexShrink={0} marginTop={Math.max(0, prefixSum[visibleStart] - scrollOffset)}/>)}
      {visibleItems.map(({ item, actualIndex }) => (<Box key={actualIndex} width="100%">
