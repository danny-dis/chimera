...
export const ModeSelector = ({ mode, onSelect, focused = false, compact = false, contentWidth, }) => {
    ...
    if (compact) {
        ...
        {MODES.map((m) => {
                const sel = m === mode;
                return (<Box key={m} marginRight={1}>
              <Text color={sel ? zen.accent : zen.muted} bold={sel} underline={sel}>
                {MODE_META[m].icon} {m}
              </Text>
            </Box>);
            })}
    ...
    return (<Box flexDirection="column" borderStyle="single" borderColor={zen.accent} paddingX={1}>
