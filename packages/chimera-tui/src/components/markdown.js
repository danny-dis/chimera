...
// ── Renderers ─────────────────────────────────────────────────────────────
const renderInlineTokens = (tokens) => tokens.map((token, i) => <InlineTokenRenderer key={i} token={token}/>);
const InlineTokenRenderer = ({ token }) => {
    switch (token.type) {
        ...
        case 'code':
            return (<Text backgroundColor={zen.muted} color={zen.fg}>
          {` ${token.value} `}
        </Text>);
        default:
            return null;
...
const BlockTokenRenderer = ({ token }) => {
    switch (token.type) {
        case 'heading': {
            ...
            return (<Box marginTop={level <= 2 ? 1 : 0} marginBottom={0}>
        ...
        case 'paragraph':
            return (<Box flexDirection="column" marginBottom={0} width="100%">
        ...
        case 'list_item': {
            ...
            return (<Box marginLeft={2} width="100%">
...
export const Markdown = ({ content }) => {
    ...
    return (<Box flexDirection="column" width="100%">
      {tokens.map((token, i) => (<BlockTokenRenderer key={i} token={token}/>))}
    </Box>);
};
