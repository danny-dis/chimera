...
export const EventLog = ({ events, filter = null, onFilterChange, focused = false, height = 10, contentWidth, }) => {
    ...
    return (<Box flexDirection="column" borderStyle="round" borderColor={focused ? zen.accent : zen.border} paddingX={1} height={height + 4}>
      <Box marginBottom={0}>
      ...
      {eventTypes.length > 1 && !filter && (<Box marginBottom={0}>
      ...
      <Viewport items={filteredEvents} height={height} focused={focused} renderItem={(event, _index, isSelected) => {
            ...
            return (<Box flexDirection="column">
              ...
              {isSelected && !isCollapsed && event.data && (<Box marginLeft={6} flexDirection="column">
                  <Text dimColor>{JSON.stringify(event.data, null, 2).slice(0, 200)}</Text>
                </Box>)}
            </Box>);
        }}/>

      {focused && (<Box marginTop={0}>
