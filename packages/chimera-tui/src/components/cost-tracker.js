...
export const CostTracker = ({ data, showBreakdown = true, contentWidth }) => {
    ...
    if (isNarrow) {
        return (<Box borderStyle="round" borderColor={zen.success} paddingX={1}>
    ...
    if (data.currentCost === 0 && data.breakdown.length === 0) {
        return (<Box flexDirection="column" borderStyle="round" borderColor={zen.success} paddingX={1}>
        <Box marginBottom={1}>
    ...
    return (<Box flexDirection="column" borderStyle="round" borderColor={zen.success} paddingX={1}>
      ...
      {showBreakdown && data.breakdown.length > 0 && (<Box flexDirection="column" marginTop={1}>
          <Text bold dimColor>
            Breakdown:
          </Text>
