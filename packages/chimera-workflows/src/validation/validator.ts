...
import { isBashNode, isLoopNode, isScriptNode } from '../schemas/dag-node.js';
...
import { isInlineScript } from '../executor/executor-shared.js';
import { buildAiProfile, resolveModelSpec } from './model-validation.js';
import type { RawAliasesConfig, RawTiersConfig, ResolvedAiProfile } from './model-validation.js';
...
function getHomeCommandsPath(): string {
  return join(homedir(), '.chimera', 'commands');
}
...
function getProviderCapabilities(_provider: string): Record<string, boolean> {
  return { mcp: false, skills: false, hooks: false, agents: false, toolRestrictions: false };
}

const { getLog, resetLog } = createLazyLogger('workflow.validator');
