...
import type { IWorkflowPlatform, WorkflowMessageMetadata, WorkflowRun } from '../deps';
...
import { isLoopNode, isApprovalNode, isScriptNode, isBashNode } from '../schemas/dag-node';
...
import { buildAiProfile, isLiteralSpec, resolveModelSpec } from '../validation/model-validation';
import type { ModelAliasPreset, ResolvedAiProfile } from '../validation/model-validation';
import { createLazyLogger } from '../logger-utils';

function captureWorkflowInvoked(_data: Record<string, unknown>): void {
  // Telemetry stub — wired when telemetry backend is available
}

function captureWorkflowCompleted(_data: Record<string, unknown>): void {
  // Telemetry stub — wired when telemetry backend is available
}

async function getDefaultBranch(_repoPath: string): Promise<string> {
  return 'main';
}

function toRepoPath(cwd: string): string {
  return cwd;
}

function isRegisteredProvider(_id: string): boolean {
  return true;
}

function getRegisteredProviders(): Array<{ id: string }> {
  return [];
}
...
const { getLog, resetLog } = createLazyLogger('workflow.executor');
  ...
  try {
    ...
    for (const f of files) {
      try {
        ...
        getLog().warn({ err: writeErr as Error, path: f.path }, 'provider_env_file_write_failed');
      }
    }
    ...
    try {
      ...
      if (codebase?.name) {
        ...
        if (parts.length === 2) {
          ...
          return {
            artifactsDir: join(cwd, '.chimera', 'artifacts', 'runs', workflowRunId),
            logDir: join(cwd, '.chimera', 'logs'),
          };
        }
...
export async function executeWorkflow(
  ...
  let fileConfig: WorkflowConfig;
  try {
    fileConfig = await deps.loadConfig(cwd);
  } catch (configErr) {
    getLog().error({ err: configErr as Error, cwd }, 'workflow.config_load_failed');
    return { success: false, error: `Failed to load config: ${(configErr as Error).message}` };
  }
  const dbEnvVars = codebaseId
    ? await deps.store.getCodebaseEnvVars(codebaseId).catch((dbErr) => {
        getLog().warn({ err: dbErr as Error, codebaseId }, 'workflow.db_env_vars_load_failed');
        return {};
      })
    : {};
  ...
  if (workflow.mutates_checkout !== false) {
    try {
        const startedAtMs = parseDbTimestamp(workflowRun.started_at);
      ...
      if (activeWorkflow) {
        ...
        const activeStartedMs = parseDbTimestamp(activeWorkflow.started_at);
  ...
  try {
    ...
    try {
      ...
      if (finalStatus?.status === 'completed') {
        ...
        getLog().warn(
          { workflowRunId: workflowRun.id },
          'workflow.run_not_found_but_dag_succeeded'
        );
        return { success: true, workflowRunId: workflowRun.id, summary: dagSummary };
      } else {
        return {
      ...
      getLog().error(
        { err: dbError as Error, workflowRunId: workflowRun.id },
        'workflow.final_status_query_failed'
      );
    ...
    getLog().error(
      { err, workflowName: workflow.name, workflowId: runId },
    ...
    if (runId) {
      try {
        ...
        getLog().error(
          { err: dbError as Error, workflowId: runId, originalError: err.message },
      ...
      try {
        await logWorkflowError(logDir, runId, err.message);
      } catch (logError) {
        getLog().error(
          { err: logError as Error, workflowId: runId },
      ...
      deps.store
        ...
        .catch((evtErr: Error) => {
          getLog().error(
            { err: evtErr, workflowRunId: runId, eventType: 'workflow_failed' },
            'workflow_event_persist_failed'
          );
        });
      ...
      if (!delivered) {
        getLog().error(
          { workflowId: runId, originalError: err.message },
