...
import type { IWorkflowPlatform, WorkflowDeps, WorkflowMessageMetadata } from '../deps.js';
import { BUNDLED_COMMANDS, isBinaryBuild } from '../defaults/bundled-defaults.js';
...
function getCommandFolderSearchPaths(_configuredFolder?: string): string[] {
  return ['.chimera/commands', '.archon/commands'];
}

function getHomeCommandsPath(): string {
  const { homedir } = require('os');
  return require('path').join(homedir(), '.chimera', 'commands');
}

function getDefaultCommandsPath(): string {
...
async function findMarkdownFilesRecursive(
  dir: string,
  _prefix: string,
  _opts?: { maxDepth?: number },
): Promise<Array<{ commandName: string; relativePath: string }>> {
  const { readdir } = require('fs/promises');
  const results: Array<{ commandName: string; relativePath: string }> = [];
  try {
...
const { getLog, resetLog } = createLazyLogger('workflow.executor-shared');
 ...
 */
export function toTelemetryErrorClass(errorType: ErrorType): WorkflowErrorClass {
  switch (errorType) {
    case 'FATAL':
      return 'fatal';
    case 'TRANSIENT':
...
export async function safeSendMessage(
  ...
  try {
    ...
    getLog().error(
      { err: err as Error, conversationId, ...context },
      'safe_send_message_failed'
    );
    return false;
  }
}
