...
export interface WorkflowDeps {
  ...
  getUserProviderEnv?(userId: string, artifactsDir: string): Promise<{ env: Record<string, string>; files: Array<{ path: string; contents: string }> }>;
}

export interface WorkflowStore {
  getCodebase(id: string): Promise<{ name: string; repository_url?: string } | null>;
  ...
  getActiveWorkflowRunByPath(path: string, opts: { id: string; startedAt: Date }): Promise<WorkflowRun | null>;
...
export interface WorkflowConfig {
  ...
  defaults?: { loadDefaultCommands?: boolean };
  tiers?: Record<string, { provider: string; model: string; effort?: string }>;
  aliases?: Record<string, { provider: string; model: string; effort?: string }>;
  assistants?: Record<string, { model?: string }>;
}
