...
exports.CONTEXT_VAR_PATTERN_STR = exports.TRANSIENT_PATTERNS = exports.FATAL_PATTERNS = exports.resetLogCacheForTests = void 0;
exports.matchesPattern = matchesPattern;
exports.classifyError = classifyError;
exports.toTelemetryErrorClass = toTelemetryErrorClass;
exports.formatSubprocessFailure = formatSubprocessFailure;
exports.detectCreditExhaustion = detectCreditExhaustion;
exports.loadCommandPrompt = loadCommandPrompt;
exports.substituteWorkflowVariables = substituteWorkflowVariables;
exports.buildPromptWithContext = buildPromptWithContext;
exports.detectCompletionSignal = detectCompletionSignal;
exports.stripCompletionTags = stripCompletionTags;
...
function getHomeCommandsPath() {
    ...
    return require('path').join(homedir(), '.chimera', 'commands');
}
function getDefaultCommandsPath() {
    return require('path').join(__dirname, '..', 'defaults');
}
    ...
    try {
        ...
        for (const entry of entries) {
            if (entry.isFile() && entry.name.endsWith('.md')) {
                ...
                results.push({ commandName, relativePath: entry.name });
            }
        }
    }
...
const { getLog, resetLog } = (0, logger_utils_js_1.createLazyLogger)('workflow.executor-shared');
...
function classifyError(error) {
    ...
    if (matchesPattern(message, exports.FATAL_PATTERNS)) {
        return 'FATAL';
    }
    if (matchesPattern(message, exports.TRANSIENT_PATTERNS)) {
        return 'TRANSIENT';
    }
    return 'UNKNOWN';
}
 ...
 */
function toTelemetryErrorClass(errorType) {
    switch (errorType) {
        case 'FATAL':
            return 'fatal';
        case 'TRANSIENT':
 ...
 */
function formatSubprocessFailure(err, label) {
    ...
    const truncated = diagnostic.length > SUBPROCESS_ERROR_MAX_CHARS
        ? diagnostic.slice(-SUBPROCESS_ERROR_MAX_CHARS) + '\n…[truncated]'
        : diagnostic;
    const exitSuffix = err.code != null ? ` [exit ${String(err.code)}]` : '';
    const stderrTail = stderr.length > SUBPROCESS_ERROR_MAX_CHARS ? stderr.slice(-SUBPROCESS_ERROR_MAX_CHARS) : stderr;
    return {
 ...
 */
async function loadCommandPrompt(deps, cwd, commandName, configuredFolder) {
    ...
    const resolvedSearchPaths = [
        ...searchPaths.map(folder => (0, path_1.join)(cwd, folder)),
        getHomeCommandsPath(),
    ];
    for (const dir of resolvedSearchPaths) {
        const entries = await findMarkdownFilesRecursive(dir, '', { maxDepth: 1 });
        const match = entries.find(e => e.commandName === commandName);
        if (!match)
            continue;
        const filePath = (0, path_1.join)(dir, match.relativePath);
        try {
    ...
    if (loadDefaultCommands) {
        if ((0, bundled_defaults_js_1.isBinaryBuild)()) {
            ...
            const bundledContent = bundled_defaults_js_1.BUNDLED_COMMANDS[commandName];
            if (bundledContent) {
            ...
            const entries = await findMarkdownFilesRecursive(appDefaultsPath, '', {
                maxDepth: 1,
            });
            const match = entries.find(e => e.commandName === commandName);
            if (match) {
                const filePath = (0, path_1.join)(appDefaultsPath, match.relativePath);
                try {
                    const content = await (0, promises_1.readFile)(filePath, 'utf-8');
                    if (!content.trim()) {
...
exports.CONTEXT_VAR_PATTERN_STR = '\\$(?:CONTEXT|EXTERNAL_CONTEXT|ISSUE_CONTEXT)(?![A-Za-z0-9_])';
/**
 ...
 */
function substituteWorkflowVariables(prompt, workflowId, userMessage, artifactsDir, baseBranch, docsDir, issueContext, loopUserInput, rejectionReason, loopPrevOutput, options) {
    ...
    if (!baseBranch && prompt.includes('$BASE_BRANCH')) {
        throw new Error('No base branch could be resolved. Auto-detection failed and `worktree.baseBranch` is not set in .archon/config.yaml. ' +
    ...
    const hasContextVariables = new RegExp(exports.CONTEXT_VAR_PATTERN_STR).test(result);
    ...
    if (!options?.shellSafe) {
        ...
        result = result.replace(new RegExp(exports.CONTEXT_VAR_PATTERN_STR, 'g'), () => issueContext ?? '');
    }
    return {
        prompt: result,
        contextSubstituted: !options?.shellSafe && hasContextVariables && !!issueContext,
    };
}
/**
 ...
 */
function buildPromptWithContext(template, workflowId, userMessage, artifactsDir, baseBranch, docsDir, issueContext, logLabel) {
    const { prompt, contextSubstituted } = substituteWorkflowVariables(template, workflowId, userMessage, artifactsDir, baseBranch, docsDir, issueContext);
...
function detectCompletionSignal(output, signal) {
    ...
    const xmlWrappedPattern = new RegExp(`<([a-zA-Z][\\w-]*)[^>]*>\\s*${escapeRegExp(signal)}\\s*</\\1>`, 'i');
 ...
 */
function stripCompletionTags(content, until) {
    ...
    if (until) {
        ...
        result = result.replace(new RegExp(`<([a-zA-Z][\\w-]*)[^>]*>\\s*${escapedSignal}\\s*</\\1>`, 'gi'), '');
    }
    return result.trim();
}
/**
...
async function safeSendMessage(platform, conversationId, message, context) {
    try {
        await platform.sendMessage(conversationId, message);
        return true;
    }
    catch (err) {
        getLog().error({ err: err, conversationId, ...context }, 'safe_send_message_failed');
        return false;
    }
}
//# sourceMappingURL=executor-shared.js.map