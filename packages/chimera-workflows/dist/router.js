...
exports.buildRouterPrompt = buildRouterPrompt;
exports.parseWorkflowInvocation = parseWorkflowInvocation;
exports.findWorkflow = findWorkflow;
exports.resolveWorkflowName = resolveWorkflowName;
const logger_utils_1 = require("./logger-utils");
const { getLog, resetLog } = (0, logger_utils_1.createLazyLogger)('workflow.router');
 ...
 */
function buildRouterPrompt(userMessage, workflows, context) {
 ...
 */
function parseWorkflowInvocation(message, workflows) {
    ...
    if (match) {
        ...
        if (caseMatch) {
            getLog().info({ requested: workflowName, matched: caseMatch.name }, 'workflow.invoke_case_insensitive_match');
            const remainingMessage = trimmed.slice(match.index + match[0].length).trim();
 ...
 */
function findWorkflow(name, workflows) {
 ...
 */
function resolveWorkflowName(name, workflows) {
    // Tier 1: Exact match
    ...
    function checkTier(matches, logEvent) {
        if (matches.length === 1) {
    ...
    checkTier(workflows.filter(w => w.name.toLowerCase() === lowerName), 'workflow.resolve_case_insensitive_match') ??
        // Tier 3: Suffix match (e.g. "assist" matches "archon-assist")
        checkTier(workflows.filter(w => w.name.toLowerCase().endsWith(`-${lowerName}`)), 'workflow.resolve_suffix_match') ??
        // Tier 4: Substring match (e.g. "smart" matches "archon-smart-pr-review")
        checkTier(workflows.filter(w => w.name.toLowerCase().includes(lowerName)), 'workflow.resolve_substring_match'));
}
