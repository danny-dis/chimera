 ...
 */
function taskWantsFiles(task) {
    if (/\b(create|scaffold|write|generate|build|implement|make|port|add|fix|correct|patch|edit|repair|refactor)\b/i.test(task))
        return true;
    // Named file anywhere in the task (mid-sentence or trailing) — covers debug.
    if (expectedPathFromTask(task))
        return true;
    if (/Cargo\.toml|src[\\/]|\b(src|lib|app|components|tests?)\b[\\/]/i.test(task))
        return true;
    return false;
}
 ...
 */
function fileLandedOnDisk(task, workspaceRoot) {
