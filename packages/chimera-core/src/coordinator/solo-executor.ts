...
export class SoloExecutor {
  ...
  async executeWithAnalysis(
    ...
    try {
      ...
      const writerConfig: SoloConfig = this.toolRegistry
      ...
      while (
        ...
        try {
          ...
          lastContent = sanitizeWriterOutput(followUp.content);
      ...
      if (wantsFiles && this.toolExecutor && this.workspaceRoot && round > 0) {
        ...
        while (realFiles < FORCE_MIN_FILES && forceAttempts < MAX_FORCE) {
          ...
          try {
            ...
            lastContent = sanitizeWriterOutput(forced.content);
  ...
  private async followUpWithToolResults(
    ...
    wroteFileCount = 0,
  ): Promise<{ content: string; inputTokens: number; outputTokens: number; toolCalls?: import('../types/agent.js').ToolCall[] }> {
