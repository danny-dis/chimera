...
export class BackgroundTaskManager extends EventEmitter {
  ...
  listTasks(filter?: { status?: BackgroundTask['status'] }): BackgroundTask[] {
    if (!filter?.status) return [...this.queue];
  ...
  async waitForTask(taskId: string, timeoutMs = 60_000): Promise<BackgroundTask> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const onComplete = (task: BackgroundTask) => {
        if (task.id === taskId) {
          clearTimeout(timer);
          this.removeListener('task_failed', onFail);
          this.removeListener('task_cancelled', onCancel);
      ...
      const onFail = (task: BackgroundTask) => {
        if (task.id === taskId) {
          clearTimeout(timer);
          this.removeListener('task_completed', onComplete);
          this.removeListener('task_cancelled', onCancel);
      ...
      const onCancel = (task: BackgroundTask) => {
        if (task.id === taskId) {
          clearTimeout(timer);
          this.removeListener('task_completed', onComplete);
          this.removeListener('task_failed', onFail);
