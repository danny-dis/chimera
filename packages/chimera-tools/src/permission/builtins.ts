 ...
 */
export function askOnOsTools(): PermissionProfile {
  return {
 ...
 */
export function readOnlyPolicy(): PermissionProfile {
  return {
    name: 'read-only',
    mode: 'readOnly',
    rules: [
 ...
 */
export function workspaceWritePolicy(): PermissionProfile {
  return {
 ...
 */
export function trustedProjectPolicy(): PermissionProfile {
  return {
 ...
 */
export function costBudgetPolicy(options: {
  maxCostUsd: number;
 ...
 */
export function maxToolCallsPolicy(limit: number): PermissionProfile {
  return {
    name: 'max-tool-calls',
    mode: 'custom',
    rules: [
 ...
 */
export function destructiveCommandsPolicy(): PermissionProfile {
  return {
 ...
 */
export function getBuiltinPolicyNames(): string[] {
  return [
    'ask-on-os-tools',
    'read-only',
    'workspace-write',
 ...
 */
export function createBuiltinPolicy(
  name: string,
  ...
  switch (name) {
    ...
    case 'cost-budget':
      return costBudgetPolicy({
        maxCostUsd: (options?.maxCostUsd as number) ?? 5.0,
        askThresholdsUsd: (options?.askThresholdsUsd as number[]) ?? [3.0],
      });
    case 'max-tool-calls':
      return maxToolCallsPolicy((options?.limit as number) ?? 50);
    case 'destructive-commands':
      return destructiveCommandsPolicy();
    case 'network-policy':
      return networkPolicy({
        allowNetwork: options?.allowNetwork as boolean,
        allowedDomains: options?.allowedDomains as string[],
      });
    default:
      return undefined;
  }
