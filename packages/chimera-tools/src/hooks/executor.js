...
class HookExecutor {
     ...
     */
    async loadFromWorkspace(workspaceRoot) {
        const hooksPath = path_1.default.join(workspaceRoot, '.chimera', 'hooks.yaml');
    ...
    async executeHooks(event, context) {
        const matching = this.hooks.filter((h) => h.enabled && h.event === event && this.matchesFilter(h, context));
        const results = [];
    ...
    executeInlineScript(script, context, timeout) {
    ...
    executeCommand(command, context, timeout, cwd) {
        return new Promise((resolve, reject) => {
            const env = {
                ...process.env,
                ...context.data,
