...
exports.ROLE_TO_DMRX_PRESET = exports.DMRX_PRESETS = void 0;
exports.isDmrxBackend = isDmrxBackend;
exports.isDmrxPreset = isDmrxPreset;
exports.applyDmrxRouting = applyDmrxRouting;
/** DMR-X dynamic-routing meta-models (verbatim from DMR-X README). */
exports.DMRX_PRESETS = [
    'auto',
    'auto-fast',
    'auto-smart',
    'auto-agentic',
...
function isDmrxBackend(backend) {
    return backend === 'dmrx';
}
function isDmrxPreset(model) {
    return exports.DMRX_PRESETS.includes(model);
}
/**
 ...
 */
function applyDmrxRouting(config, backend, mode) {
    if (!isDmrxBackend(backend))
        return config;
    const modePreset = mode ? MODE_TO_DMRX_PRESET[mode] : undefined;
    const providers = config.providers.map((p) => {
        const rolePreset = p.role ? exports.ROLE_TO_DMRX_PRESET[p.role] : undefined;
        // Mode bias wins for non-core roles; core roles keep their tuned preset.
        const preset = rolePreset && (p.role === 'writer' || p.role === 'reviewer' || p.role === 'challenger')
            ? rolePreset
...
//# sourceMappingURL=dmrx-routing.js.map