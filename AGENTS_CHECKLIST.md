...
- **2026-08-02 (opencode)**: Hook timeout + test/hygiene pass. (1) **Hook timeout enforcement** — `packages/chimera-tools/src/hooks/executor.ts` dropped `spawn()`'s `timeout` option; both private spawn paths (`executeInlineScript`, `executeCommand`) now run a manual `setTimeout` that `child.kill('SIGKILL')`s. The first attempt relied on the `error` handler's `err.message.includes('killed')`, which was dead code — a killed child emits `close` with `code === null`, not `error`, so timeouts surfaced as a misleading "exited with code null". Added a `timedOut` flag (set in the timer, honored in both `close` and `error`) so the killed path deterministically rejects with "Hook script/command timed out after Xms" (verified empirically on Windows before/after). Regression test: `packages/chimera-tools/src/__tests__/executor.test.ts` (2 tests: normal resolve + timeout-message). (2) **`scripts/smoke-test.js`** — `timeoutMs` hoisted above the `try` block (previously declared after first use). (3) **Daemon teardown** — `server.test.ts` `afterEach` now wraps `daemon.dispose()` in try/catch and `fs.rm` in `.catch()`, so teardown errors can't mask the following test. (4) **Git hygiene** — untracked `hello.txt` smoke-test artifact removed from the repo root; `git worktree prune` + `git worktree remove --force .chimera-worktrees/agent-uniqueslug9` cleared all 16 stale linked-worktree registrations (15 were already prunable temp dirs), then all 16 merged `chimera-agent-proof-ag-*` / `chimera-agent-task-*` / `chimera-agent-uniqueslug9` branches were deleted with `git branch -d`. Test totals: chimera-tools 261 passed/1 skipped, chimera-daemon 23/23; `tsc --noEmit` clean for both packages.
- **2026-08-03 (opencode)**: **LSP tool now live.** (1) **`@chimera/lsp` server-config** — default TS/JS scoping when a server config has no `filePatterns` (previously matched everything), plus the `?` wildcard now maps to `[^/]` (single non-slash char) in `matchesPattern` and `**/` matches zero directories so root-level files (e.g. `index.ts`) match `**/*.{ts,...}` (61/61 tests). (2) **`chimera-tools/src/lsp-registry.ts` (new)** — module-level registry of `ChimeraLspService`s keyed by resolved workspace root. `resolveDefaultLspConfig` loads the workspace's `.chimera/config.yaml` `lsp:` section and, when it declares no servers, falls back to a default TypeScript server: `node <typescript-language-server cli.mjs> --stdio`, TS/JS `filePatterns` + `package.json`/`tsconfig.json` `rootFiles`. `configPath` is wired so an explicit `lsp:` config wins over the default; server children are disposed on `process.on('exit')`. (3) **`lsp` tool + `getDiagnosticsForFile` now use the registry** — previously inert because empty default servers (`DEFAULT_LSP_CONFIG.servers = {}`) matched no file, so every operation returned "No results found". (4) **Edit-sync** — `write_file`, `edit_block`, `search_replace`, and `apply_patch` call `updateDocument` after successful mutations so tsserver stays in sync. (5) **Daemon parity** — the daemon's `SessionOrchestrator` now receives the `lspDiagnostics` hook (matching `cli-router.ts`), so daemon sessions emit `lsp_diagnostics` events after edits. (6) **`typescript-language-server` moved to runtime dependencies** of `@chimera/tools`. (7) **Proof** — `lsp-tool.test.ts` now exercises `lspTool.execute` end-to-end against the real server (documentSymbol → goToDefinition across files → hover on a function call), 6/6 passing.
...
| F2 | Incomplete | **LSP tool returns hardcoded mock data** for all 5 operations (goToDefinition, findReferences, hover, documentSymbol, workspaceSymbol). Agents receive fabricated results. — **Resolved 2026-08-03**: the `lsp` tool now executes real LSP operations through `@chimera/lsp` (JSON-RPC client + server lifecycle), defaulting to a TypeScript language server (`typescript-language-server --stdio` via node) when the workspace configures none; edit tools sync documents after writes; server children are disposed on process exit. | [x] COMPLETE | `chimera-tools/src/tools/lsp.ts`, `chimera-tools/src/lsp-registry.ts` |
